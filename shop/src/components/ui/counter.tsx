...
import Spinner, { SpinnerLoader } from './loaders/spinner/spinner';
...
const variantClasses = {
  helium:
    ` h-18 items-center w-20 sm:h-7 md:h-6 md:w-[128px] bg-accent  sm:flex-row  sm:static  ltr:right-3 rtl:left-3 sm:bottom-0 ltr:sm:right-0 ltr:sm:left-0 text-light rounded`,
...
const Counter: React.FC<CounterProps> = ({
  ...
  if(isLoading){
    return <div className=' flex justify-center'><SpinnerLoader /></div>
  }
  ...
  return (
    ...
    >
      <button
        onClick={onDecrement}
        className={cn(
          'cursor-pointer p-1 md:p-2 transition-colors duration-200 hover:bg-accent-hover focus:outline-0',
          {
      ...
      <div
        className={cn(
          'flex flex-1 items-center justify-center px-[6px] md:px-3 text-sm font-semibold',
